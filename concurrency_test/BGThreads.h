...
public:
    ...
    int AddSchedule(function_t func, void *arg);
    void StartAll();
private:
    enum ThreadState {
        Empty = 0x01,
    ...
    struct CtrlArg {
        BGThreads *BGptr;
        CtrlArg(BGThreads *b) : BGptr(b){ }
    };
    ThreadType ctrl_;
    static void* CtrlWork(void *arg);
    Mutex ctrl_lock;
    CondVar ctrl_cv_; //CondVar for ctrl thread.
    bool shouldstop_;
    ...
    CondVar cv_; //CondVar for user threads.
